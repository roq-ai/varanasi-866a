...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiMap } from 'react-icons/fi';

import { TourismInterface } from 'interfaces/tourism';
import { getTourisms } from 'apiSdk/tourisms';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<TourismInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/tourisms` : null),
    () => getTourisms({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Tourisms',
      path: isTenantUser ? `/tourisms/view/${data?.[0]?.id}` : '/tourisms',
      entity: 'tourism',
