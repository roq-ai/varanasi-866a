...
import { getTourisms, deleteTourismById } from 'apiSdk/tourisms';
import { TourismInterface } from 'interfaces/tourism';

type ColumnType = ColumnDef<TourismInterface, unknown>;

interface TourismListPageProps {
...
export function TourismListPage(props: TourismListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TourismInterface>>(
    () => `/tourisms?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TourismInterface) => {
    if (hasAccess('tourism', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/tourisms/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/tourisms/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('tourism', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/tourisms/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('tourism', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
